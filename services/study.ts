...
export async function getTopicBySlug(slug: string) {
  ...
  try {
    const supabase = await createServerSupabaseClient();
    // Fetch the requested topic and the FULL library (all categories + all topics)
    // together so the workspace sidebar can list every category and its topics.
    const [{ data: topicRow, error }, library] = await Promise.all([
      supabase.from('study_topics').select(topicSelect).eq('slug', slug).eq('published', true).maybeSingle(),
      getStudyLibrary(),
    ]);
    if (error || !topicRow) return { library, topic: undefined, category: undefined };
    const topic = toTopic(topicRow as TopicRow);
    const { data: categoryRow } = topic.categoryId
      ? await supabase.from('study_categories').select('id,name,slug,description,icon,sort_order').eq('id', topic.categoryId).eq('published', true).maybeSingle()
      : { data: null };
    const category = categoryRow ? {
      ...
      moduleCount: 0, topicCount: library.topics.filter((item) => item.categoryId === categoryRow.id).length,
    } : undefined;
    return { library, topic, category };
  } catch (error) {
...
export async function getTopicContent(slug: string) {
  if (!configured()) return { topic: undefined, category: undefined, error: 'Supabase is not configured.' };
  try {
    const supabase = await createServerSupabaseClient();
    // Lightweight fetch used by the client-side topic switcher (content-only updates).
    const { data: topicRow, error } = await supabase.from('study_topics').select(topicSelect).eq('slug', slug).eq('published', true).maybeSingle();
    if (error || !topicRow) return { topic: undefined, category: undefined, error: error?.message ?? 'Topic not found.' };
    const topic = toTopic(topicRow as TopicRow);
    const { data: categoryRow } = topic.categoryId
      ? await supabase.from('study_categories').select('id,name,slug,description,icon,sort_order').eq('id', topic.categoryId).eq('published', true).maybeSingle()
      : { data: null };
    ...
    return { topic: undefined, category: undefined, error: error instanceof Error ? error.message : 'Unable to load topic.' };
  }
}
...
export async function searchStudy(query: string) {
  ...
  const supabase = await createServerSupabaseClient();
  const q = `%${query.trim()}%`;
  const [categories, topics, courses] = await Promise.all([
  ...
  return { categories: categories.data ?? [], topics: topics.data ?? [], courses: courses.data ?? [], dsa: [] };
}
