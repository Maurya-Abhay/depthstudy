...
export async function getDsaProblems(){try{const supabase=createAdminSupabaseClient();const{data,error}=await supabase.from('dsa_problems').select(select).eq('published',true).order('title');if(error)return{problems:[],error:error.message};return{problems:(data??[]).map((row: Row)=>mapProblem(row))}}catch(error){return{problems:[],error:error instanceof Error?error.message:'Unable to load DSA problems.'}}}
export async function getDsaProblem(slug:string){try{const supabase=createAdminSupabaseClient();const{data,error}=await supabase.from('dsa_problems').select(select).eq('slug',slug).eq('published',true).maybeSingle();if(error||!data)return{problem:undefined,error:error?.message};return{problem:mapProblem(data as Row)}}catch(error){return{problem:undefined,error:error instanceof Error?error.message:'Unable to load DSA problem.'}}}
export async function getDsaTopics(){try{const supabase=createAdminSupabaseClient();const{data}=await supabase.from('dsa_topics').select('id,name,slug,description').eq('published',true).order('sort_order').order('name');return data??[]}catch{return []}}
...
export async function getDsaNavigationProblems(){
  try {
    ...
    return{problems:(data??[]).map((row: {id:string;topic_id:string|null;title:string;slug:string;difficulty:string}) => ({id:row.id,topicId:row.topic_id??undefined,title:row.title,slug:row.slug,difficulty:row.difficulty}))};
  }catch(error){
