...
export function rateLimitForUser(request: Request, userId: string, options: Omit<RateLimitOptions, 'key'>) {
  return rateLimit(request, { ...options, key: `user:${userId}` });
}


export function assertServerEnvironment() {
  if (typeof window !== 'undefined') {
    throw new Error('This operation is server-only.');
  }
}

export function isSafeRedirectTarget(value: string | null | undefined) {
