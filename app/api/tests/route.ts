...
async function checkEligibility(supabase: Awaited<ReturnType<typeof createServerSupabaseClient>>, userId: string, testId: string) {
...
export async function GET(request: Request) {
  ...
  const result = await checkEligibility(admin, user.id, id);
  if (result.error) return NextResponse.json({ error: result.error }, { status: result.status });
  const { data, error } = await admin.from('test_questions').select('question_id,sort_order,questions(id,prompt,type,options)').eq('test_id', id).order('sort_order');
...
export async function POST(request: Request) {
  ...
  const result = await checkEligibility(admin, user.id, testId);
  if (result.error) return NextResponse.json({ error: result.error }, { status: result.status });

  if (body.action === 'start') {
    const { data: submittedAttempts, count: submittedCount } = await admin.from('test_attempts').select('id', { count: 'exact', head: true }).eq('user_id', user.id).eq('test_id', testId).not('submitted_at', 'is', null);
    ...
    const { data: existing } = await admin.from('test_attempts').select('id,started_at').eq('user_id', user.id).eq('test_id', testId).is('submitted_at', null).order('started_at', { ascending: false }).limit(1).maybeSingle();
    if (existing) return NextResponse.json({ attempt: existing });
    const { data: attempt, error } = await admin.from('test_attempts').insert({ user_id: user.id, test_id: testId }).select('id,started_at').single();
  ...
  const { data: attempt } = await admin.from('test_attempts').select('id,started_at,submitted_at').eq('id', attemptId).eq('user_id', user.id).eq('test_id', testId).single();
  ...
  const { data: assignedRows } = await admin.from('test_questions').select('question_id').eq('test_id', testId);
  ...
  if (questionEntries.length) {
    const { error } = await admin.from('test_answers').upsert(questionEntries, { onConflict: 'attempt_id,question_id' });
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
  }

  const suspicious = Array.isArray(body.suspiciousEvents) ? body.suspiciousEvents.slice(0, 50) : [];
  if (body.action === 'save') {
    await admin.from('test_attempts').update({ suspicious_events: suspicious }).eq('id', attemptId).eq('user_id', user.id).is('submitted_at', null);
    return NextResponse.json({ ok: true });
  }

  const { data: questions } = await admin.from('test_questions').select('question_id,questions(answer)').eq('test_id', testId);
  ...
  if (answerUpdates.length) await Promise.all(answerUpdates.map((item) => admin.from('test_answers').update({ is_correct: item.is_correct }).eq('attempt_id', attemptId).eq('question_id', item.question_id)));
  const { data: submittedAttempt, error: updateError } = await admin.from('test_attempts')
    .update({ submitted_at: new Date().toISOString(), score, passed, suspicious_events: suspicious })
    .eq('id', attemptId)
    .eq('user_id', user.id)
    .is('submitted_at', null)
    .select('id')
    .maybeSingle();
  if (updateError) return NextResponse.json({ error: 'Unable to finalize this assessment. Please try again.' }, { status: 400 });
  if (!submittedAttempt) return NextResponse.json({ error: 'This assessment has already been submitted.' }, { status: 409 });

  let certificateCode: string | null = null;
  if (passed && result.test?.course_id) {
    const { data: course } = await admin.from('courses').select('certificate_enabled,slug').eq('id', result.test.course_id).single();
