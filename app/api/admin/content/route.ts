...
async function admin() {
  ...
  return { supabase: createAdminSupabaseClient(), user };
}
...
export async function POST(request: Request) {
  ...
  if (body.action === 'bulk') {
    ...
    for (let index = 0; index < records.length; index += 1) {
      try {
        built.push(recordFor(kind, records[index] as Record<string, unknown>));
      } catch (error) {
        const message = error instanceof Error ? error.message : 'Invalid record.';
        return NextResponse.json({ error: `Record ${index + 1}: ${message}` }, { status: 400 });
      }
    }
    // DSA problems have a unique slug — re-importing the same problem updates it
    // (useful for fixing/assigning topics) instead of failing on a duplicate slug.
    const query = kind === 'dsa'
      ? supabase.from(tableFor(kind)).upsert(built, { onConflict: 'slug' })
      : supabase.from(tableFor(kind)).insert(built);
